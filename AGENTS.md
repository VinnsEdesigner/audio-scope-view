...
## Web app wiring: dsp-wasm end-to-end (architecture migration, Step 4 complete)
- **Single DSP source of truth in the browser:** the web app no longer has a duplicate TS FFT. `apps/vyzorWeb/src/lib/scope-dsp.ts` (radix-2 FFT, `computeSpectrum`, `toDecibels`, `triggeredWindow`, `findTriggerIndex`, `resampleTo`) is **deleted**. All DSP now flows through `@audio-scope-view/dsp-wasm` (the C++ core compiled to WASM). The web app's `AudioScopeDsp` calls are the browser/worklet analog of the Rust `dsp_ffi.rs` — same C ABI, same core.
- **Singleton loader (`apps/vyzorWeb/src/lib/dsp-loader.ts`):** `ensureDsp()` (async, kicks off + awaits the WASM load, memoized, failure-tolerant → returns null) and `getDsp()` (sync, returns the loaded instance or null). Shared by `scope-canvas.tsx`, `use-audio-analyzer.ts`, `use-mock-audio-analyzer.ts`, `measurements-dialog.tsx`. The WASM module is expensive to instantiate (fetch + compile); it must load exactly once. Hot-path callers (rAF draw loop, capture tick) read `getDsp()` and degrade gracefully (free-run / inline fallback) until loaded — the UI never blocks on module load.
- **Consumers wired:** (1) `scope-canvas.tsx` — `drawSpectrum` now takes a precomputed WASM `Spectrum` (dB magnitudes + frequencies, drawn directly without the old `toDecibels` step); `triggeredWindow` calls `dsp.triggeredWindow` (returns `Float32Array | null`); `ensureDsp()` on mount. The Canvas2D renderer is retained (the WebGL2 rewrite is §C.1, a separate task). (2) `use-audio-analyzer.ts` — RMS/peak/frequency via `dsp.computeRms`/`findPeakAmplitude`/`estimateDominantFrequency` (with tiny inline TS fallbacks pre-load). (3) `measurements-dialog.tsx` — Vpp/RMS/DC-offset/frequency via the WASM core. (4) `use-mock-audio-analyzer.ts` — the TS `generateSample` synthesizer is replaced by `dsp.generateWaveform` (C++ sine/square/saw/triangle/noise generators); a `useEffect` regenerates the buffer when frequency/amplitude/waveformType/sampleRate change.
...
- **Type compatibility:** the ui-store's `triggerEdge: "rising" | "falling" | "auto"` is structurally identical to the WASM `TriggerEdge`, so no adapter is needed. `frame` in `scope-canvas.tsx` is now `ArrayLike<number>` (the WASM `triggeredWindow` returns `Float32Array`; held frames are `number[]`), so the `for...of` loop was converted to an index loop (`ArrayLike` is not `Iterable` in strict TS).
...
- **Browser verification:** a throwaway `public/wasm-smoke.html` imported the loader directly and exercised the WASM core: 440 Hz sine → `computeSpectrum` peak ~422 Hz (within FFT bin resolution 48000/1024=46.875 Hz), `computeRms` 0.7099 (expect ~0.707), `findPeakAmplitude` 1.0, `estimateDominantFrequency` ~422 Hz (zero-crossing estimator, same algorithm as old TS `calculateFrequency`), `triggeredWindow` len 256, `generateWaveform(square)` len 512 first=0.8. The smoke file is deleted after verification (not committed).
- **Remaining §C work (NOT done in this step):** §C.1 WebGL2 renderer (`src/lib/webgl/` — gl-context.ts, scope-renderer.ts, spectrum-renderer.ts, shaders/) is a separate graphics rewrite; the Canvas2D path is kept for now. §C.3 also mentions repointing the playback worklet (`use-streaming-playback.ts`) to the dsp-wasm processor — the analyzer path (done) was the priority per the spec.
- **Branch:** `feat/cpp-dsp-core-rust-ffi-migration`. Step 4 = commit `5f7ab61` (wire dsp-wasm into web app + delete scope-dsp.ts). All suites green: 12 WASM vitest, 36 native C++ SDK, 31 Rust, vyzorWeb tsc clean.
