...
impl TursoApiKeyRepository {
    ...
    fn parse_datetime(s: &str) -> AppResult<std::time::SystemTime> {
        ...
        Err(crate::shared::error_app::AppError::validation(&format!("Invalid datetime format: {}", s)))
    }

    fn row_to_api_key(row: &[crate::infrastructure::turso_http_client::TursoValue]) -> AppResult<ApiKey> {
        // Columns: id, key_hash, name, rate_limit_per_minute, expires_at, last_used_at, created_at
        let created_at_str = row.get(6).and_then(|v| v.as_str())
        ...
        Ok(ApiKey {
            ...
            expires_at: row.get(4).and_then(|v| v.as_str()).filter(|s| !s.is_empty())
                .and_then(|s| Self::parse_datetime(s).ok()),
            rate_limit_per_minute: row.get(3).and_then(|v| v.as_i64()).unwrap_or(60) as u32,
            last_used_at: row.get(5).and_then(|v| v.as_str()).filter(|s| !s.is_empty())
                .and_then(|s| Self::parse_datetime(s).ok()),
        })
    }
}
...
impl ApiKeyRepository for TursoApiKeyRepository {
    async fn save(&self, api_key: &ApiKey) -> AppResult<()> {
        ...
        self.client.execute_void_with_args(sql, vec![
            ...
            TursoArg::opt_text(api_key.expires_at.map(|t| Self::format_datetime(t))),
            TursoArg::opt_text(api_key.last_used_at.map(|t| Self::format_datetime(t))),
    ...
    async fn find_by_key(&self, key: &str) -> AppResult<Option<ApiKey>> {
        ...
        let result = self.client.execute_with_args(sql, vec![TursoArg::text(key_hash)])
            .await.map_err(Self::map_err)?;
    ...
    async fn list_all_with_hash(&self) -> AppResult<Vec<ApiKeyWithHash>> {
        ...
        if let Some(TursoResult::Ok(ok)) = result.results.first() {
            for row in &ok.response.result.rows {
                ...
                let key_hash = row.get(1).and_then(|v| v.as_str()).unwrap_or("").to_string();
