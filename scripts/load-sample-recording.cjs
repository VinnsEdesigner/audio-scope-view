#!/usr/bin/env node
/**
 * Script to load a 10MB audio sample recording into the database
 * Generates a realistic sine wave audio sample and inserts it via the GraphQL API
 */
...
function generateAudioSamples(sampleCount, sampleRate = 44100, frequency = 440) {
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    // Main sine wave + harmonics + noise
    const sine = Math.sin(2 * Math.PI * frequency * t);
    const harmonic = 0.3 * Math.sin(2 * Math.PI * frequency * 2 * t); // 2nd harmonic
...
async function createRecording() {
  // Audio parameters - target ~10MB of samples (Float32 = 4 bytes each)
  // 10MB = 10 * 1024 * 1024 bytes / 4 bytes per sample = ~2.6 million samples
  const sampleRate = 44100;
  const targetSizeMB = 10;
  const targetSizeBytes = targetSizeMB * 1024 * 1024;
  const sampleCount = Math.floor(targetSizeBytes / 4); // Float32 = 4 bytes
  ...
  const samples = generateAudioSamples(sampleCount, sampleRate, 440); // 440Hz sine wave
  ...
  // GraphQL mutation to create recording
  const mutation = `
