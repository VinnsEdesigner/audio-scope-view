...
function extractErrorMessage(error: unknown): string {
  ...
  if (formatted.includes("UNCAUGHT_ERROR") || formatted.includes("INTERNAL_SERVER_ERROR")) {
...
export function DialogMicRecording({
  ...
  // Custom discard-confirmation dialog state (replaces native window.confirm)
  ...
  const handleClose = () => {
    // If there is an unsaved or active recording, show a custom confirm dialog
    // instead of the native browser confirm() before discarding.
    if (stoppedSamples !== undefined || recordingState !== "idle") {
 ...
 `,
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            <div className="text-center p-2 bg-bg-elevated rounded-lg">
              <div className="text-sm font-semibold font-mono text-foreground">
                {recordingState === "idle"
                  ? stoppedSamples
                    ? formatBytes(stoppedSamples.length * 4)
                    : "0 B"
                  : formatBytes(samples.length * 4)}
              </div>
              <div className="text-[10px] text-text-tertiary uppercase">Size</div>
            </div>
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        message="You have an unsaved recording. Are you sure you want to discard it? This cannot be undone."
